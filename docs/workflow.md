...
**进度评论策略**：从 `gcw-issue-prepare` 起，每个主步骤在关键节点完成时 **新发一条** `<!-- gcw-progress -->` 评论（禁止编辑旧评论）；`refs.progress_comment_url` 指向最新评论。发布使用 `publish_progress_comment.py`；事件记录同时保存 `progress_comment_body_hash`，远程校验比对的是最新评论正文与当前 `phase` 的渲染结果及 body hash。
