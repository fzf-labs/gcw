...
description: Convert a ready GCW issue into an issue branch with state.json and spec files under .gcw/issues/<issue-id>/. Use when an issue is ready-for-planning.
---
...
- Write `.gcw/issues/<issue-id>/state.json`.
...
3. Initialize `state.json` in the issue directory with the issue identity, branch, owner, and status history.
4. Commit only the initial spec/state files as a focused planning commit, then push the issue branch without force pushing.
