...
def latest_prepare_event(issue_dir: Path) -> dict[str, Any] | None:
...
def verify_github(repo: str, issue: str, prepare: dict[str, Any]) -> list[str]:
    errors: list[str] = []
...
def verify_gitlab(repo: str, issue: str, prepare: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    payload = prepare.get("payload") if isinstance(prepare.get("payload"), dict) else {}
    classification = payload.get("classification") if isinstance(payload.get("classification"), dict) else {}
    labels_applied = payload.get("labels_applied") if isinstance(payload.get("labels_applied"), list) else []
...
def verify_issue(
    ...
    *,
    ...
    prepare = latest_prepare_event(issue_dir)
    if prepare is None:
        return {"ok": False, "errors": ["no gcw-issue-prepare event found"]}
    ...
    if resolved_platform == "github":
        errors = verify_github(resolved_repo, resolved_issue, prepare)
    else:
        errors = verify_gitlab(resolved_repo, resolved_issue, prepare)
