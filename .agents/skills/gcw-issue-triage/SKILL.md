...
| Workflow labels | `labels_applied` | labels (`area:*`, `triaged`, optional) | labels |
...
`triage_options.json` must include `classification_type`, `classification_priority`, `labels_applied`, and `remote_sync_file`. Include `classification_area` when known.
...
```json
{
  ...
  "labels_applied": ["triaged", "area:workflow"],
  "remote_sync": {
    "platform": "github",
    "issue_type": "Feature",
    "priority": "Medium",
